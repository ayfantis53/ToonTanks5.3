...
auto ATT_Game_mode::start_game() -> void
{
	...
	if (GEngine && GEngine->GameViewport)
	{
		...
		view_port->AddViewportWidgetContent(SAssignNew(start_game_widget_container_, SWeakWidget).PossiblyNullContent(start_game_widget_ref_));
...
auto ATT_Game_mode::game_over(bool b_won_game) -> void
{
	...
	if (GEngine && GEngine->GameViewport)
	{
		...
		view_port->AddViewportWidgetContent(SAssignNew(end_game_widget_container_, SWeakWidget).PossiblyNullContent(end_game_widget_ref_));
...
auto ATT_Game_mode::handle_game_start() -> void
{
	...
	toon_tanks_player_controller_ref_ = Cast<ATT_Player_controller>(UGameplayStatics::GetPlayerController(this, 0));

	start_game();

	// Set Timer that counts down Game Start.
	if (toon_tanks_player_controller_ref_)
	{
		toon_tanks_player_controller_ref_->set_player_enabled_state(false);

		// Call another classes function to enable player control.
		FTimerHandle player_enable_timer_handle;
		FTimerDelegate player_enable_timer_delegate = FTimerDelegate::CreateUObject(toon_tanks_player_controller_ref_, &ATT_Player_controller::set_player_enabled_state, true);
